...
bool es_capicua(int n){
    int a=n, b=0;
    llista_itr<int> toni;
    llista_itr<int>::iterador itPrimer = toni.ultim();
    while(a!=0)
    {

        b=a%10;
        a=a/10;
        toni.inserir_davant(b, itPrimer);
        itPrimer++;

    }

    itPrimer=toni.primer();
    llista_itr<int>::iterador itUltim = toni.ultim();

    for(unsigned int i = 0; i < toni.longitud()/2;i++){
        if(*itPrimer != *itUltim){
            return false;
        }
        itPrimer++;
