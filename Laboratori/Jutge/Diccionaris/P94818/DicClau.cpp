...
template<typename Clau>
void DicClau<Clau>::print() const {
    cout << "[";
    print_iter(_arrel);
    cout << "]" << endl;
}
template<typename Clau>
void DicClau<Clau>::print_iter(DicClau::node *n) {
    if(n != NULL){
        print_iter(n->_esq);
        cout<< n->_k << ' ';
        print_iter(n->_dret);
...
template<typename Clau>
DicClau<Clau>::DicClau(const DicClau &d) {
    *this = d;
}

template<typename Clau>
void DicClau<Clau>::elimina(DicClau::node *n) {
    if(n != NULL){
        print_iter(n->_esq);
        delete(n);
        print_iter(n->_dret);
...
template<typename Clau>
DicClau<Clau> &DicClau<Clau>::operator=(const DicClau &d) {
...
template<typename Clau>
void DicClau<Clau>::copia(DicClau::node *n1, DicClau::node *n2) {
    if(n2->_esq != NULL) {
        n1->_esq = new node(n2->_esq->_k);
        print_iter(n1->_esq, n2->_esq);
    }
    if(n2->_dret != NULL) {
        n1->_dret = new node(n2->_dret->_k);
        print_iter(n1->_dret, n2->_dret);
    }
}
