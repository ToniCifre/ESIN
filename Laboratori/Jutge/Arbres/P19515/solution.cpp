...
private:
    ...
    void preordre_especular(node* p1, node* p2) ;
    nat get_max_prof(node* p1, nat prof);
public:
    node* _arrel;
...
template <typename T>
nat Arbre<T>::get_max_prof(){
    get_max_prof(_arrel, 0);
}

template <typename T>
nat Arbre<T>::get_max_prof(node* p, nat prof){
    if(p!= NULL){
        prof = get_max_prof(p->primf, ++prof);
        prof = get_max_prof(p->seggerm, prof);
    }
    return prof;
}
...
void Arbre<T>::preordre_especular(node* p){
    preordre_especular(_arrel, p);
}

template <typename T>
void Arbre<T>::preordre_especular(node* p1, node* p2){
    if(p1!= NULL & p2 != NULL){
        cout<<" "<< p1->info + p2->info;
        preordre_especular(p1->primf,p2->primf);
        preordre_especular(p1->seggerm, p2->seggerm);
    }else if(p1!= NULL & p2 == NULL){
        cout<<" "<< p1->info;
        preordre_especular(p1->primf, p2);
        preordre_especular(p1->seggerm, p2);
    } else if(p1== NULL & p2 != NULL){
        cout<<" "<< p2->info;
        preordre_especular(p1, p2->primf);
        preordre_especular(p1, p2->seggerm);
    }
}
